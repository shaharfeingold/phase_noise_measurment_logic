...
module system_ctrl(clk, rstn, clken, start_op, finish_op, event_start_op_when_system_not_ready,
                   ...
                   out_addr_ch0, out_addr_ch1, data_count_ch0, data_count_ch1, phase_inc_vld, state_start);
//parameters
...
wire config_done = (delay_counter == 4'b1111);
...
wire [DATA_WIDTH-1 : 0] in_data_ch0_d; 
wire in_data_vld_ch0_d;
wire [DATA_WIDTH-1 : 0] in_data_ch1_d; 
wire in_data_vld_ch1_d;
...
always @(posedge clk or negedge rstn) begin
    if (~rstn) begin
        ...
        state <= state_ns;
        delay_counter <= (state == CONFIG) ? delay_counter + 4'b0001 : 4'b0000; 
        ...
        fifo_size_ch1 <= in_data_vld_ch1_d & fifo_not_full_ch1 ? fifo_size_ch1 + {{FIFO_SIZE_WIDTH-1{1'b0}},1'b1} :  
                     (state == IDLE) || (state == WAIT_FOR_START) ? {FIFO_SIZE_WIDTH{1'b0}} : fifo_size_ch1;
