module async_fifo(rstn, rd_clk, read_req, data_out_vld, data_out, fifo_empty, wr_clk, data_in_vld, data_in, fifo_full);
//parameters
parameter DATA_WIDTH = 32;
parameter FIFO_DEPTH = 8;
parameter integer FIFO_DEPTH_WIDTH = $clog2(FIFO_DEPTH);

//ports
input rstn; //generate a rest for each domain with it clock speratly
// read i/f
input rd_clk;
input read_req;
output data_out_vld;
...
always @(posedge rd_clk ) begin
    if (~rstn) begin
        ...
        read_ptr <= (~empty_pp & read_req) ? read_ptr_next : read_ptr;
        data_out_p <= (~empty_pp & read_req) ? memory[read_ptr[FIFO_DEPTH_WIDTH-2:0]] : {DATA_WIDTH{1'b0}};
        data_out_vld_p <= (~empty_pp & read_req) ? 1'b1 : 1'b0;
        empty_p <= empty_pp;
    end
...
wire [3 : 0] write_ptr_gray;
wire [3 : 0] write_ptr_next_gray;
...
wire [3: 0] write_ptr_gray_rd_sync;
...
wire [3 : 0] read_ptr_gray_wr_sync;
...
module sync(rstn, clk, data_in, data_out);
//parameters
...
input rstn; //active high
input clk;

//implementation
reg [FIFO_DEPTH_WIDTH: 0] shiftreg [0:1];
...
always @(posedge clk ) begin
    if (~rstn) begin
//        for ( i = 0 ; i < SYNC_WIDTH ; i = i + 1) begin
            shiftreg[0] <= {(FIFO_DEPTH_WIDTH+1){1'b0}};
            shiftreg[1] <= {(FIFO_DEPTH_WIDTH+1){1'b0}};
//        end
    end
    else begin
//        for ( j = 0 ; j < SYNC_WIDTH-1 ; j = j + 1) begin
            {shiftreg[1],shiftreg[0]} <= {shiftreg[1], data_in};
//        end
//        shiftreg[0] <= data_in;
    end
end
//endgenerate
assign data_out = shiftreg[1];
